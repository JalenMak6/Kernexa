...
export default function App() {
  ...
  const HostTable = ({ showSortArrows = false }) => (
    <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
      <thead style={{ background: "#f8fafc", borderBottom: "1px solid #f1f5f9" }}>
        <tr>
          <th style={thStyle("host")} onClick={() => sortBy("host")}>Host {showSortArrows && sortCol === "host" ? (sortDir === "asc" ? "↑" : "↓") : ""}</th>
          <th style={{ ...thStyle("os_version"), width: 120 }} onClick={() => sortBy("os_version")}>OS {showSortArrows && sortCol === "os_version" ? (sortDir === "asc" ? "↑" : "↓") : ""}</th>
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ position: "fixed", left: 0, top: 0, bottom: 0, width: 220, background: "#0f172a", display: "flex", flexDirection: "column", borderRight: "1px solid #1e293b", zIndex: 50 }}>
        ...
        <nav style={{ padding: "16px 12px", flex: 1, overflowY: "auto" }}>
          ...
          {[
            { label: "Inventories",  icon: "file",    badge: activeInventoryName ? "active" : null, onClick: () => setShowInventoryManager(true) },
      ...
      <div style={{ marginLeft: 220, padding: "32px", minHeight: "100vh", width: "calc(100vw - 220px)", overflowX: "hidden" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button onClick={triggerScan} disabled={scanDisabled} title={scanTooltip} style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: scanDisabled ? "#e2e8f0" : "linear-gradient(135deg,#3b82f6,#6366f1)", color: scanDisabled ? "#94a3b8" : "#fff", fontWeight: 700, fontSize: 12, cursor: scanDisabled ? "not-allowed" : "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6, boxShadow: scanDisabled ? "none" : "0 2px 8px rgba(59,130,246,0.35)" }}>
              {scanning
        ...
        {activeInventoryName && !activeHasCredentials && !error && (
          <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 10, padding: "12px 16px", marginBottom: 20, color: "#c2410c", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button onClick={() => setShowInventoryManager(true)} style={{ background: "#c2410c", color: "#fff", border: "none", borderRadius: 6, padding: "6px 14px", cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: "inherit", whiteSpace: "nowrap", marginLeft: 12 }}>Set Credentials</button>
          </div>
        )}
        {error && (
        ...
        {/* ── Tab: Settings ── */}
        {tab === "settings" && <div style={{ animation: "fadeIn 0.3s ease" }}><SettingsTab /></div>}
                    ...
                    return (
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 16, marginBottom: 24 }}>
                        {["Critical", "Important", "Moderate", "Low"].map(sev => {
                          ...
                          return (
                            <div key={sev} onClick={() => changeTab("cves")} style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, borderRadius: 12, padding: "16px 20px", cursor: "pointer", transition: "all 0.15s", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}
                  ...
                  <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                    <div style={{ padding: "16px 20px", borderBottom: "1px solid #f1f5f9" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                        ...
                        {(activeFilterCount > 0 || search) && <button onClick={clearFilters} style={{ padding: "5px 12px", borderRadius: 6, border: "1px solid #fca5a5", background: "#fef2f2", cursor: "pointer", fontSize: 12, color: "#dc2626", fontFamily: "inherit" }}>Clear all ×</button>}
                      </div>
                      <FilterBar />
                    </div>
                    <HostTable />
                          ...
                          : history.map(s => (
                            <tr key={s.scan_id} style={{ borderBottom: "1px solid #f1f5f9" }}
                              ...
                              <td style={{ padding: "14px 16px" }}>
                                {s.failure_count > 0
                                  ? <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", borderRadius: 6, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: "#ef4444" }} />{s.failure_count} failed</span>
                                  : <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "#f0fdf4", color: "#16a34a", border: "1px solid #86efac", borderRadius: 6, padding: "3px 10px", fontSize: 12, fontWeight: 600 }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: "#22c55e" }} />All OK</span>
                                }
                              </td>
                              <td style={{ padding: "14px 16px" }}>
                                <button onClick={() => setSelectedScanId(s.scan_id)} style={{ padding: "5px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600, border: "1px solid #e2e8f0", background: "#f8fafc", color: "#475569", cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 5 }}
