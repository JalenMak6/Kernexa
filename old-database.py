...
def get_inventories():
    ...
    cursor = conn.cursor()
    try:
        # CHANGED: added COALESCE(i.inventory_type, 'linux') to SELECT
        cursor.execute('''
        ...
        return [
            {
                ...
                'is_active':       row[4],
                'inventory_type':  row[5],        # CHANGED: added
                'has_credentials': row[6]          # CHANGED: index shifted
            }
            for row in rows
        ]
    finally:
...
def save_inventory(name: str, content: str, host_count: int,
    ...
    # CHANGED: added inventory_type param (default 'linux' for backwards compat)
    conn = get_conn()
    cursor = conn.cursor()
    try:
        cursor.execute('''
...
def activate_inventory(inv_id: int) -> str:
    # CHANGED: only deactivates inventories of the SAME type
    # so Linux + Windows can both be active simultaneously
    conn = get_conn()
    cursor = conn.cursor()
    try:
        # Get the type of the inventory being activated
        cursor.execute(
        ...
        # Only deactivate inventories of the same type
        cursor.execute('''
...
def get_active_inventory_credentials() -> dict | None:
    # CHANGED: now filters to linux inventory only so Windows-active
    # inventory doesn't accidentally get used for a Linux scan
    conn = get_conn()
    cursor = conn.cursor()
    try:
        cursor.execute('''
...
# ADDED: helper to get the active Linux inventory metadata
def get_active_linux_inventory() -> dict | None:
    conn = get_conn()
    cursor = conn.cursor()
    try:
        cursor.execute('''
            SELECT id, name, content, host_count
            FROM inventories
...
# ADDED: helper to get the active Windows inventory metadata
...
def save_windows_to_db(output: dict, scan_id: str):
    """
    Persists Windows patch scan results into windows_scan_results.
    One row per pending KB per host.
    scan_id must already exist in scan_runs (created by the Linux scan
    or a dedicated Windows scan_run insert).
    """
    conn = get_conn()
    cursor = conn.cursor()
    try:
        for host, data in output['hosts'].items():
            ...
            for update in data.get('updates', []):
                # Parse ISO timestamps — store NULL if blank or unparseable
                def _parse_ts(val):
                    if not val:
                        return None
                    try:
...
def get_latest_windows_scan() -> list:
    """
    Returns all Windows patch records from the most recent scan_run
    that has any windows_scan_results rows, including the scan timestamp.
    """
    conn = get_conn()
    cursor = conn.cursor()
    try:
...
def get_latest_scan():
    ...
    cursor = conn.cursor()
    try:
        # CHANGED: only return scan_runs that have Linux scan_results rows
        # so a Windows-only scan run doesn't show as "latest" on the Linux dashboard
        cursor.execute('''
